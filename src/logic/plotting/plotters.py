...
class OneDimensionalPlotter(ODEPlotter):
    """
    Plotter for 1D time-series (e.g., decay system transients).

    Creates time-series plots showing state variables evolution over time.
    Useful for single state variables or individual components from multi-D systems.
    """
    ...
    def plot(
        ...
        y : NDArray[np.float64]
            State values, shape (1, n_points)
        labels : Optional[list[str]]
            Labels for [x-axis (time), y-axis (state vars)]
        title : Optional[str]
        ...
        with plt.style.context(config.style):
            ...
            ax.plot(t, y[0, :], linewidth=config.line_width, color=color, alpha=config.alpha)

            # Mark start and end points for transient validation
            if config.show_markers: 
                ax.plot(t[0], y[0, 0], "go", 
                    markersize=config.marker_size, label = "Start")
                ax.plot(t[-1], y[0, -1], "ro", 
...
class ThreeDimensionalPlotter(ODEPlotter):
